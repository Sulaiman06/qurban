<x-app-layout>
    ...
    <div class="py-12">
        ...
        <div class="flex items-center justify-end mt-4">
                <form action="{{ route('congregants.create') }}" method="GET">
                      ...
                      @foreach ( $congs as $qurban )
                        <tbody class="flex-1">
                            <tr>
                              ...
                              <form action="{{ route('congregants.edit', $qurban->id) }}" method="GET">
                                <x-primary-button class="ml-4 mb-4 bg-green-700">
                                    {{ __('Edit') }}
                                </x-primary-button>
                              </form>
                              <form action="{{ route('congregants.destroy', $qurban->id) }}" method="POST">
                                  @csrf
                                  @method('DELETE')
                                <x-primary-button class="ml-4 mb-4 bg-green-700">
                                    {{ __('Delete') }}
