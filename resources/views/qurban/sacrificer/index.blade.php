    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex items-center justify-end mt-4">
                <form action="{{ route('sacrificers.create') }}" method="GET">
                    <x-primary-button class="ml-4 mb-4 bg-green-700">
                        {{ __('Tambah Shohibul Qurban') }}
                    </x-primary-button>
                </form>
            </div>
                              ...
                              @foreach ( $sacs as $qurban )
                                <tbody class="flex-1">
                                    <tr>
                                      ...
                                      <form action="{{ route('sacrificers.edit', $qurban->id) }}" method="GET">
                                        <x-primary-button class="ml-4 mb-4 bg-green-700">
                                            {{ __('Edit') }}
                                        </x-primary-button>
                                      </form>
                                      <form action="{{ route('sacrificers.destroy', $qurban->id) }}" method="POST">
                                          @csrf
                                          @method('DELETE')
                                        <x-primary-button class="ml-4 mb-4 bg-green-700">
                                            {{ __('Delete') }}
